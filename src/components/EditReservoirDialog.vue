                ...
                <el-form-item label="工程档案" prop="projectArchive" required>
                  <div class="file-upload-container">
                    ...
                    <div v-if="basicForm.projectArchive" class="uploaded-file">
                      <span 
                        class="file-name" 
                        :class="{ 'file-downloadable': basicForm.projectArchive.url }"
        ...
        <div v-show="activeTab === 'dam'" class="tab-content">
          <el-form ref="damFormRef" :model="damForm" :rules="damRules" label-width="120px" label-position="left">
            ...
            <el-row> <el-col :span="24"> <el-form-item label="主坝类型(按结构)" prop="structureType" required class="long-label-form-item"> <el-select
                    v-model="damForm.structureType" placeholder="请选择" style="width: 100%"> <el-option label="重力坝"
                      value="重力坝" />
                ...
                :span="12">
                <el-form-item label="主坝类型(按材料)"> <el-select v-model="damForm.materialType" placeholder="请选择"
                  ...
                  <el-select v-model="damForm.spillwayType" placeholder="请选择" style="width: 100%"> <el-option
...
import { addReservoir, getReservoirDetail, updateReservoir, getPresignedObjectUrl } from '@/api/reservoir';
...
// 文件修改状态追踪
const fileModificationState = reactive({
  engineeringDocModified: false, // 工程档案是否修改
  originalImagesList: [], // 原始图片列表（用于比较）
  deletedImages: [], // 删除的图片ID列表
  newImages: [] // 新增的图片列表
});
...
const resetAllForms = () => {
  ...
  // 重置文件修改状态
  fileModificationState.engineeringDocModified = false;
  fileModificationState.originalImagesList = [];
  fileModificationState.deletedImages = [];
  fileModificationState.newImages = [];
};
...
const handleSave = async () => {
  try {
    ...
    const requestData = {
      ...
      reservoirDamInfoUpDTO: {
        id: props.editData.reservoirDamInfoVO?.id || '', // 添加大坝信息ID
        ...
        reservoirManagementId: props.editData.id // 编辑时传入水库ID
      },

      reservoirEigenvalueInfoUpDTO: {
        id: props.editData.reservoirEigenvalueInfoVO?.id || '', // 添加特征值信息ID
    ...
    if (props.editData.id) {
      // 编辑模式下处理工程档案修改流
      if (fileModificationState.engineeringDocModified) {
        if (basicForm.projectArchive && basicForm.projectArchive.content && basicForm.projectArchive.content.startsWith('data:')) {
          // 有新文件上传，engineeringDoc为空，engineeringDocFlow包含base64
          requestData.engineeringDoc = '';
          requestData.engineeringDocFlow = basicForm.projectArchive.content;
        } else {
          // 文件被删除，两个字段都为空
          requestData.engineeringDoc = '';
          requestData.engineeringDocFlow = '';
        }
      } else {
        // 工程档案未修改，保持原有路径，flow为空
        requestData.engineeringDoc = basicForm.projectArchive?.originalPath || '';
        requestData.engineeringDocFlow = '';
      }
    } else {
      // 新增模式
    ...
    if (props.editData.id) {
      ...
      // 1. 添加未删除的原有图片
      fileModificationState.originalImagesList.forEach(originalImg => {
        if (!fileModificationState.deletedImages.includes(originalImg.id)) {
          picInfoDTOS.push({
            id: originalImg.id,
            reservoirManagementId: originalImg.reservoirManagementId,
            file: originalImg.originalPath,
            fileFlow: '' // 原有图片未修改，flow为空
          });
        }
      });
      
      // 2. 添加新增的图片
      fileModificationState.newImages.forEach(newImg => {
        picInfoDTOS.push({
          id: '', // 新增图片ID为空
          reservoirManagementId: props.editData.id,
          file: '', // 新增图片file为空
          fileFlow: newImg.url // 新增图片的base64放在flow中
        });
      });
      
      requestData.reservoirFilePicInfoDTOS = picInfoDTOS;
    } else {
      // 新增模式
    ...
    // 调用API
    if (props.editData.id) {
      await updateReservoir(requestData);
    } else {
    ...
    // 显示成功提示
    ElMessage({
      message: props.editData.id ? '更新水库成功！' : '新增水库成功！',
      type: 'success',
      duration: 2000
    });
...
const handleImageUpload = async (file) => {
  try {
    ...
    // 添加到新增图片列表
    fileModificationState.newImages.push(newImage);
...
const handleFileUpload = async (file) => {
  ...
  try {
    ...
    // 标记工程档案已修改
    fileModificationState.engineeringDocModified = true;
  } catch (error) {
...
const deleteCurrentImage = () => {
  ...
  const deletedImage = imagesList.value[currentImageIndex.value];
  
  // 如果是原有图片（有id），记录到删除列表
  if (deletedImage.id) {
    fileModificationState.deletedImages.push(deletedImage.id);
  }
  
  // 如果是新增图片，从新增列表中移除
  if (deletedImage.isNew) {
    const newIndex = fileModificationState.newImages.findIndex(img => img === deletedImage);
    if (newIndex > -1) {
      fileModificationState.newImages.splice(newIndex, 1);
    }
  }
...
const deleteImageFromThumbnails = (index) => {
  const deletedImage = imagesList.value[index];
  ...
  // If the deleted image was active and not the last one, the next image (or first if last was deleted) 
  // will automatically become active due to index shifting or resetting, no specific action needed for this case here.
};
...
// 水库类型反向映射函数（从中文转换为表单值）
const getReservoirTypeValue = (chineseType) => {
  const typeMap = {
    '大(一)型': 'large_type_1',
    '大(二)型': 'large_type_2', 
    '中型': 'middle_type',
...
const fillFormData = async (data) => {
  ...
  if (data.engineeringDoc) {
    const fileUrl = await getFilePresignedUrl(data.engineeringDoc);
    projectArchive = { 
      name: data.engineeringDoc.split(',')[1] || data.engineeringDoc, // 使用文件名部分
      content: fileUrl, // 使用预签名URL作为content以通过验证
      url: fileUrl, // 预签名URL用于下载
      originalPath: data.engineeringDoc // 保存原始路径
    };
  }
  ...
  let latitude = '';
  if (data.lonLat) {
    const coords = data.lonLat.split(',');
    if (coords.length === 2) {
  ...
  Object.assign(basicForm, {
    name: data.reservoirName || '',
    code: data.reservoirCode || '',
    type: getReservoirTypeValue(data.reservoirType) || '',
    catchmentArea: data.catchmentArea?.toString() || '',
    ...
    longitude: longitude,
    latitude: latitude,
    buildTime: data.websiteCrtTime || '',
    manager: data.manageUnit || '',
    river: data.belongRiverName || '',
    adminResponsible: data.administrativeResponsiblePerson || '',
    techResponsible: data.technicalResponsiblePerson || '',
    inspectionResponsible: data.patrolResponsiblePerson || '',
    projectArchive: projectArchive,
    remark: data.remarks || ''
  });

  // 根据省市区名称反向查询code并填充级联选择
  await fillCascadingSelection(data);

  // 填充大坝信息
  if (data.reservoirDamInfoVO) {
    const damInfo = data.reservoirDamInfoVO;
    Object.assign(damForm, {
      height: damInfo.mainDamHeight?.toString() || '',
      length: damInfo.mainDamLength?.toString() || '',
      structureType: damInfo.mainDamTypeStructure || '',
      materialType: damInfo.mainDamTypeMaterial || '',
      spillwayType: damInfo.mainTypesFloodDischargeStructures || '',
      averageRunoff: damInfo.annualAverageRunoff?.toString() || '',
      remark: damInfo.reamrks || ''
    });
  }
  ...
  if (data.reservoirEigenvalueInfoVO) {
    ...
    Object.assign(featuresForm, {
      floodSeasonLevel: eigenInfo.mainFloodSeasonWaterLevel || '',
      floodSeasonTime: eigenInfo.mainSeasonWaterLevelStart && eigenInfo.mainSeasonWaterLevelEnd 
        ? [eigenInfo.mainSeasonWaterLevelStart, eigenInfo.mainSeasonWaterLevelEnd] : '',
      postFloodLevel: eigenInfo.postFloodSeasonWaterLevel || '',
      postFloodTime: eigenInfo.postSeasonWaterLevelStart && eigenInfo.postSeasonWaterLevelEnd
        ? [eigenInfo.postSeasonWaterLevelStart, eigenInfo.postSeasonWaterLevelEnd] : '',
      deadLevel: eigenInfo.deadWaterLevel || '',
      deadCapacity: eigenInfo.deadStorageCapacity || '',
      normalLevel: eigenInfo.normalStorageLevel || '',
      beneficialCapacity: eigenInfo.utilizableCapacity || '',
      floodControlLevel: eigenInfo.floodControlHighWaterLevel || '',
      floodControlCapacity: eigenInfo.floodControlStorageCapacity || '',
      designFloodLevel: eigenInfo.designFloodLevel || '',
      checkFloodLevel: eigenInfo.checkFloodLevel || '',
      damCrestElevation: eigenInfo.damCrestElevation || '',
      totalCapacity: eigenInfo.totalStorageCapacity || '',
      historicalHighestLevel: eigenInfo.highestLevelRecords || '',
      historicalHighestTime: eigenInfo.highestAppearanceTime || '',
      historicalMaxStorage: eigenInfo.maximumStorage || '',
      historicalMaxStorageTime: eigenInfo.maximumStorageTime || '',
      historicalMaxInflow: eigenInfo.maximumInflow || '',
      historicalMaxInflowTime: eigenInfo.maximumInflowTime || '',
      historicalMaxOutflow: eigenInfo.maximumOutflow || '',
      historicalMaxOutflowTime: eigenInfo.maximumOutflowTime || '',
      historicalLowestLevel: eigenInfo.lowestReservoirLevel || '',
      historicalLowestTime: eigenInfo.lowestAppearanceTime || '',
      historicalMinInflow: eigenInfo.historicalMinimumDailyInflow || '',
      historicalMinInflowTime: eigenInfo.dailyInflowAppearanceTime || ''
    });
  }

  // 填充调度原则
  if (data.reservoirSchedulingPrincipleInfoVOS && data.reservoirSchedulingPrincipleInfoVOS.length > 0) {
    dispatchForm.rules = data.reservoirSchedulingPrincipleInfoVOS.map(item => ({
      content: item.schedulingPrincipleName || ''
    }));
    dispatchForm.remark = data.reservoirSchedulingPrincipleInfoVOS[0]?.remarks || '';
  } else {
    // 如果没有调度原则数据，重置为默认状态
    dispatchForm.rules = [{ content: '' }];
    dispatchForm.remark = '';
  }

  // 填充图片信息
  if (data.reservoirFilePicInfoVOS && data.reservoirFilePicInfoVOS.length > 0) {
    const imagePromises = data.reservoirFilePicInfoVOS.map(async (item) => {
      const imageUrl = await getFilePresignedUrl(item.file);
      return {
        id: item.id, // 保存图片ID
        reservoirManagementId: item.reservoirManagementId,
        url: imageUrl || item.file, // 使用预签名URL，如果失败则使用原始路径
        name: `图片_${item.id}`,
        file: null,
        originalPath: item.file, // 保存原始路径
        isNew: false // 标记为原有图片
      };
    });
    
    imagesList.value = await Promise.all(imagePromises);
    // 保存原始图片列表的副本
    fileModificationState.originalImagesList = JSON.parse(JSON.stringify(imagesList.value));
    currentImageIndex.value = 0;
  } else {
    imagesList.value = [];
    fileModificationState.originalImagesList = [];
...
// 根据省市区名称填充级联选择
const fillCascadingSelection = async (data) => {
  try {
    // 首先加载省份列表
    ...
    if (data.province) {
      ...
      if (province) {
        basicForm.province = province.name;
        
        // 加载城市列表
        const cities = await getCityList(province.code);
        ...
        if (data.city) {
          ...
          if (city) {
            basicForm.city = city.name;
            
            // 加载区县列表
            const areas = await getAreaList(city.code);
...
const handleFileDownload = () => {
  if (basicForm.projectArchive && basicForm.projectArchive.url) {
    downloadFile(basicForm.projectArchive.url, basicForm.projectArchive.name);
  }
};
</script>
