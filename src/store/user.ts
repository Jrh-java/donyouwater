/*
 * 登录用户
 *
 * @Author:    1024创新实验室-主任：卓大
 * @Date:      2022-09-06 20:55:09
 * @Wechat:    zhuda1024
 * @Email:     lab1024@163.com
 * @Copyright  1024创新实验室 （ https://1024lab.net ），Since 2012
 */
import _ from 'lodash';
import { defineStore } from 'pinia';
// 定义常量
const HOME_PAGE_NAME = '/main/dashboard';
const MENU_TYPE_ENUM = {
  CATALOG: { value: 'M', desc: '目录' },
  MENU: { value: 'C', desc: '菜单' },  
  POINTS: { value: 'F', desc: '按钮' }
};

// 定义菜单项类型
interface MenuItem {
  id?: string;
  menuId?: number;
  parentId?: number | string;
  menuName?: string;
  menuCode?: string;
  menuType?: number | string;
  url?: string;
  component?: string;
  permission?: string;
  icon?: string;
  status?: string;
  visibleFlag?: boolean;
  disabledFlag?: boolean;
  children?: MenuItem[];
  menuTitle?: string;
  fromMenuName?: string;
  fromMenuQuery?: any;
  menuQuery?: any;
  menuIcon?: string;
}

// 定义标签页类型
interface TagNavItem {
  menuName: string;
  menuTitle: string;
  menuQuery?: any;
  menuIcon?: string;
  fromMenuName?: string;
  fromMenuQuery?: any;
}

// 定义用户信息类型
interface UserInfo {
  token?: string;
  employeeId?: string;
  avatar?: string;
  loginName?: string;
  actualName?: string;
  phone?: string;
  departmentId?: string;
  departmentName?: string;
  needUpdatePwdFlag?: boolean;
  administratorFlag?: boolean;
  lastLoginIp?: string;
  lastLoginIpRegion?: string;
  lastLoginUserAgent?: string;
  lastLoginTime?: string;
  menuTree?: MenuItem[];
}


export const useUserStore = defineStore({
  id: 'userStore',
  state: () => ({
    token: '' as string,
    //员工id
    employeeId: '' as string,
    // 头像
    avatar: '' as string,
    //登录名
    loginName: '' as string,
    //姓名
    actualName: '' as string,
    //手机号
    phone: '' as string,
    //部门id
    departmentId: '' as string,
    //部门名词
    departmentName: '' as string,
    //是否需要修改密码
    needUpdatePwdFlag: false as boolean,
    //是否为超级管理员
    administratorFlag: true as boolean,
    //上次登录ip
    lastLoginIp: '' as string,
    //上次登录ip地区
    lastLoginIpRegion: '' as string,
    //上次登录 设备
    lastLoginUserAgent: '' as string,
    //上次登录时间
    lastLoginTime: '' as string,
    //左侧菜单树形结构
    menuTree: [] as MenuItem[],
    //存在页面路由的菜单集合
    menuRouterList: [] as MenuItem[],
    //是否完成menuRouter初始化
    menuRouterInitFlag: false as boolean,
    //父类菜单集合
    menuParentIdListMap: new Map() as Map<string, any[]>,

    // 标签页
    tagNav: null as TagNavItem[] | null,
    // 缓存
    keepAliveIncludes: [] as string[],
    // 未读消息数量
    unreadMessageCount: 0 as number,
    // 待办工作数
    toBeDoneCount: 0 as number,
  }),
  getters: {
    getToken(state) {
      if (state.token) {
        return state.token;
      }
      return localStorage.getItem('USER_TOKEN');
    },
    getNeedUpdatePwdFlag(state){
      return state.needUpdatePwdFlag;
    },
    //是否初始化了 路由
    getMenuRouterInitFlag(state) {
      return state.menuRouterInitFlag;
    },
    //菜单树
    getMenuTree(state) {
      return state.menuTree;
    },
    //菜单的路由
    getMenuRouterList(state) {
      return state.menuRouterList;
    },
    //菜单的父级id
    getMenuParentIdListMap(state) {
      return state.menuParentIdListMap;
    },

    //标签页
    getTagNav(state) {
      if (_.isNull(state.tagNav)) {
        let localTagNav = localStorage.getItem('USER_TAG_NAV') || '';
        state.tagNav = localTagNav ? JSON.parse(localTagNav) : [];
      }
      let tagNavList = _.cloneDeep(state.tagNav) || [];
      tagNavList.unshift({
        menuName: HOME_PAGE_NAME,
        menuTitle: '首页',
      } as TagNavItem);
      return tagNavList;
    },
  },

  actions: {
    logout() {
      this.token = '';
      this.menuTree = [];
      this.menuRouterList = [];
      this.tagNav = [] as TagNavItem[];
      this.unreadMessageCount = 0;
      localStorage.removeItem('USER_TOKEN');

      localStorage.removeItem('USER_TAG_NAV');
      localStorage.removeItem('APP_CONFIG');
      localStorage.removeItem('HOME_QUICK_ENTRY');
      localStorage.removeItem('NOTICE_READ');
      localStorage.removeItem('TO_BE_DONE');
      
      // 权限缓存现在由Pinia store管理，在logout时会自动清除
    },
    // 查询未读消息数量
    async queryUnreadMessageCount() {
      try {
        // TODO: 实现消息API调用
        this.unreadMessageCount = 0;
      } catch (e: any) {
        console.error('Error:', e);
      }
    },
    async queryToBeDoneList() {
      try {
        let localToBeDoneList = localStorage.getItem('TO_BE_DONE');
        if (localToBeDoneList) {
          this.toBeDoneCount = JSON.parse(localToBeDoneList).filter((e: any) => !e.doneFlag).length;
        }
      } catch (err: any) {
        console.error('Error:', err);
      }
    },
    //设置登录信息
    setUserLoginInfo(data: UserInfo) {
      // 权限缓存现在由Pinia store管理，会在登录时自动加载
      
      // 用户基本信息
      this.token = data.token || '';
      this.employeeId = data.employeeId || '';
      this.avatar = data.avatar || '';
      this.loginName = data.loginName || '';
      this.actualName = data.actualName || '';
      this.phone = data.phone || '';
      this.departmentId = data.departmentId || '';
      this.departmentName = data.departmentName || '';
      this.needUpdatePwdFlag = data.needUpdatePwdFlag || false;
      this.administratorFlag = data.administratorFlag || false;
      this.lastLoginIp = data.lastLoginIp || '';
      this.lastLoginIpRegion = data.lastLoginIpRegion || '';
      this.lastLoginUserAgent = data.lastLoginUserAgent || '';
      this.lastLoginTime = data.lastLoginTime || '';

      // 处理菜单数据 - 适配新的数据结构
      const menuList = this.flattenMenuTree(data.menuTree || []);
      
      //菜单权限
      this.menuTree = data.menuTree || [];

      //拥有路由的菜单
      this.menuRouterList = menuList.filter((e) => e.url && e.url !== '#');

      //父级菜单集合
      this.menuParentIdListMap = buildMenuParentIdListMap(this.menuTree);

      // 权限检查现在通过 privilege 指令直接调用 queryMenuTree 接口处理

      // 获取用户未读消息
      this.queryUnreadMessageCount();
      // 获取待办工作数
      this.queryToBeDoneList();
    },

    // 扁平化菜单树
    flattenMenuTree(menuTree: MenuItem[]): MenuItem[] {
      const result: MenuItem[] = [];
      const flatten = (menus: MenuItem[]) => {
        menus.forEach((menu: MenuItem) => {
          result.push(menu);
          if (menu.children && menu.children.length > 0) {
            flatten(menu.children);
          }
        });
      };
      flatten(menuTree);
      return result;
    },

    setToken(token: string) {
      this.token = token;
    },

    //设置标签页
    setTagNav(route: any, from: any) {
      if (_.isNull(this.tagNav)) {
        let localTagNav = localStorage.getItem('USER_TAG_NAV') || '';
        this.tagNav = localTagNav ? JSON.parse(localTagNav) : [];
      }
      // name唯一标识
      let name = route.name;
      if (!name || name === HOME_PAGE_NAME || name === '403' || name === '404') {
        return;
      }
      let findTag = (this.tagNav || []).find((e) => e.menuName === name);
      if (findTag) {
        // @ts-ignore
        findTag.fromMenuName = from.name;
        findTag.fromMenuQuery = from.query;
      } else {
        (this.tagNav as TagNavItem[]).push({
          menuName: name,
          menuTitle: route.meta.title,
          menuQuery: route.query,
          menuIcon: route.meta?.icon,
          fromMenuName: from.name,
          fromMenuQuery: from.query,
        } as TagNavItem);
      }
      localStorage.setItem('USER_TAG_NAV', JSON.stringify(this.tagNav));
    },
    //关闭标签页
    closeTagNav(menuName: string, closeAll: boolean) {
      if (_.isEmpty(this.getTagNav)) return;
      if (closeAll && !menuName) {
        this.tagNav = [];
        this.clearKeepAliveIncludes();
      } else {
        let findIndex = (this.tagNav || []).findIndex((e) => e.menuName === menuName);
        if (closeAll) {
          if (findIndex === -1) {
            this.tagNav = [];
            this.clearKeepAliveIncludes();
          } else {
            let tagNavElement = (this.tagNav || [])[findIndex];
            this.tagNav = [tagNavElement];
            this.clearKeepAliveIncludes(tagNavElement.menuName);
          }
        } else {
          (this.tagNav || []).splice(findIndex, 1);
          this.deleteKeepAliveIncludes(menuName);
        }
      }
      localStorage.setItem('USER_TAG_NAV', JSON.stringify(this.tagNav));
    },
    //关闭页面
    closePage(route: any, router: any, path?: string) {
      if (!this.getTagNav || _.isEmpty(this.getTagNav)) return;
      if (path) {
        router.push({ path });
      } else {
        // 寻找tagNav
        let index = this.getTagNav.findIndex((e) => e.menuName === route.name);
        if (index === -1) {
          router.push({ name: HOME_PAGE_NAME });
        } else {
          let tagNav = this.getTagNav[index];
          if (tagNav.fromMenuName && this.getTagNav.some((e) => e.menuName === tagNav.fromMenuName)) {
            router.push({ name: tagNav.fromMenuName, query: tagNav.fromMenuQuery });
          } else {
            // 查询左侧tag
            let leftTagNav = this.getTagNav[index - 1];
            router.push({ name: leftTagNav.menuName, query: leftTagNav.menuQuery });
          }
        }
      }
      this.closeTagNav(route.name, false);
    },
    // 加入缓存
    pushKeepAliveIncludes(val: string) {
      if (!val) {
        return;
      }
      if (!this.keepAliveIncludes) {
        this.keepAliveIncludes = [];
      }
      if (this.keepAliveIncludes.length < 30) {
        let number = this.keepAliveIncludes.findIndex((e) => e === val);
        if (number === -1) {
          this.keepAliveIncludes.push(val);
        }
      }
    },
    // 删除缓存
    deleteKeepAliveIncludes(val: string) {
      if (!this.keepAliveIncludes || !val) {
        return;
      }
      let number = this.keepAliveIncludes.findIndex((e) => e === val);
      if (number !== -1) {
        this.keepAliveIncludes.splice(number, 1);
      }
    },
    // 清空缓存
    clearKeepAliveIncludes(val?: string) {
      if (!val || !this.keepAliveIncludes.includes(val)) {
        this.keepAliveIncludes = [];
        return;
      }
      this.keepAliveIncludes = [val];
    },
  },
});

/**
 * 构建菜单父级集合
 */
function buildMenuParentIdListMap(menuTree: MenuItem[]) {
  let menuParentIdListMap = new Map();
  recursiveBuildMenuParentIdListMap(menuTree, [], menuParentIdListMap);
  return menuParentIdListMap;
}

function recursiveBuildMenuParentIdListMap(menuList: MenuItem[], parentMenuList: any[], menuParentIdListMap: Map<string, any[]>) {
  for (const e of menuList) {
    // 顶级parentMenuList清空
    if (e.parentId === 0) {
      parentMenuList = [];
    }
    let menuIdStr = (e.menuId || e.id || '').toString();
    let cloneParentMenuList = _.cloneDeep(parentMenuList);
    if (!_.isEmpty(e.children) && e.menuName) {
      // 递归
      cloneParentMenuList.push({ name: menuIdStr, title: e.menuName });
      recursiveBuildMenuParentIdListMap(e.children || [], cloneParentMenuList, menuParentIdListMap);
    } else {
      menuParentIdListMap.set(menuIdStr, cloneParentMenuList);
    }
  }
}

/**
 * 构建菜单树
 *
 * @param  menuList
 * @returns
 */
function buildMenuTree(menuList: MenuItem[]): MenuItem[] {
  //1 获取所有 有效的 目录和菜单
  let catalogAndMenuList = menuList.filter((menu) => menu.menuType !== MENU_TYPE_ENUM.POINTS.value && menu.visibleFlag && !menu.disabledFlag);

  //2 获取顶级目录
  let topCatalogList = catalogAndMenuList.filter((menu) => menu.parentId === 0);
  for (const topCatalog of topCatalogList) {
    buildMenuChildren(topCatalog, catalogAndMenuList);
  }
  return topCatalogList;
}

function buildMenuChildren(menu: MenuItem, allMenuList: MenuItem[]) {
  let children = allMenuList.filter((e) => e.parentId === (menu.menuId || menu.id));
  if (children.length === 0) {
    return;
  }
  menu.children = children;
  for (const item of children) {
    buildMenuChildren(item, allMenuList);
  }
}
